...
export function solveLinearSystem(matrix: boolean[][], vector: boolean[]): boolean[] | "unsolvable" {
        ...
        if (trueIndex == undefined) {
            for (let j = i; j < size; j++) {
                if (m[j].includes(true)) {
                    // console.log("this system is not solvable. 1 in every " + Math.pow(2, size - i) + " random boards are solvable.");
                    return "unsolvable";
                }
            }
